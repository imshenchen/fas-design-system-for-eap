...
- **`corp`**：Delta 企業樣式 —— 預設使用 Delta 企業標誌（`DeltaLogo`）；Logo 與 App Name 採 Corp 品牌前景色 `--corp-brand-fg`（Light = Delta 藍 `#0087dc`、**Dark Mode 轉白 `#ffffff`**），兩者間加垂直分隔線；下緣為品牌三段色帶（`--delta-blue` → `--delta-brand-cyan` → `--delta-brand-lime`，三段實色硬邊）。右側內建按鈕為 information / theme / language / notification / avatar（前三者採 Delta 企業圖示 `CorpIcon`，藍色重點色 Dark Mode 提亮為 `#a5e2fa`），按鈕間距 24px；可用 `actions` / `showDefaults` 客製，亦可傳 `logo` 覆寫預設標誌。
- Corp 漸層用到的新 Token：`--delta-brand-cyan`（#64d7d7）、`--delta-brand-lime`（#b9eb5f），與既有 `--delta-blue` 同屬 Brand 色，無 Dark Mode 變體。

---

## DeltaLogo
Delta 企業品牌標誌（三角標 + DELTA 字標），以 inline SVG 提供。`NavigationBar` 的 `corp` 樣式預設使用此標誌；也可單獨用於 Footer、登入頁等情境。標誌主色走 `--corp-brand-fg`（Light = Delta 藍、**Dark Mode 轉白 `#ffffff`**），三角內凹槽走 `--bg-surface`（隨主題顯示底色），因此在深色背景上會呈現白色標誌、凹槽透出底色。
...
- **底部 footer**：
  - **`default` 樣式**：原本的版本列（`deployed_code` 圖示 + 版號，`--primary` 藍）；`versionAction`（如 `LMMobileInstallButton`）顯示在右側。**不受 corp 改動影響**。
  - **`corp` 樣式**：左側產品 Logo（`productLogo`；未指定時預設 `DiaAutoLogo`）＋右側版本（tag 圖示 `sell` + 版號；圖示 `--secondary`、版號 `--text-high`）。
    - **收折時 footer 只顯示產品 Logo（置中）、不顯示版本**。
    - **收折 hover**：hover 模組會浮出與 SideMenu 等高的「固定整條」面板（取代 default 的小浮層），頂部為模組標題＋功能列表、**底部顯示版本**。
  - `version` 字串固定顯示在底部，不隨選單捲動。

---

## CorpIcon
Delta 企業圖示集（24×24，兩色：灰 `#727171` + Delta 藍重點色）。藍色走 `--corp-icon-accent` token，Light Mode `#0087dc`、**Dark Mode 自動提亮為 `#a5e2fa`** 以維持可讀性。主要供 `SideMenu variant="corp"` 與 corp NavigationBar 以名稱引用，也可單獨使用。
...
`CorpIconName`：
- SideMenu 模組：`ai_service`、`alert_management`、`application`、`data_processing`、`image_file`、`log`、`low_code_tool`、`module`、`node`、`settings`、`storage_space`、`system_monitoring`
- NavigationBar 操作：`information`、`theme`、`notification`
...
## DiaAutoLogo
DIA Auto 產品標誌（彩色 burst，inline SVG）。為 `SideMenu variant="corp"` footer 的**預設產品 Logo**；也可單獨使用或透過 `productLogo` 覆寫。

```tsx
<DiaAutoLogo />              {/* 預設 44×44 */}
<DiaAutoLogo height={28} width={28} />
```

| Prop | Type | Default | 說明 |
|------|------|---------|------|
| `width` / `height` | `number \| string` | `44` | 尺寸 |
| `title` | `string` | `'DIA Auto'` | 無障礙標籤 |
| `...rest` | `SVGProps` | — | 其餘 SVG 屬性 |
...
| `productLogo` | `ReactNode` | `DiaAutoLogo`（corp） | **僅 `variant="corp"` 生效**；SideMenu footer 左側產品 Logo，收折時只顯示此 Logo。未指定時 corp 預設用 `DiaAutoLogo` |
